...
const client = require('twilio')(
    'ACc71279d73f17e821dbc560d7cec01889',
    '001d6ca7473d394fec8101cb2845ab9d'
)
...
app.post('/images', upload.single('profilePhoto'), async (req, res) => {
...
app.get('/getAttendance', async (req, res) => {
    if (req.query) {
        let id = parseFloat(req.query.userId)
        let atendanceDetails = await attendance
            .find({ userId: id })
            .sort({ date: 1 })
        if (atendanceDetails) {
            res.send({
...
app.get('/todayAttendanceDetails', async (req, res) => {
    if (req.query) {
        ...
        let atendanceDetails = await attendance.find({
            $and: [{ userId: id }, { date: date }],
        })
        if (atendanceDetails) {
            res.send({
                message: 'Success',
...
app.put('/updateAttendanceDetails', async (req, res) => {
    if (req.query) {
        let rollNumber = parseInt(req.query.rollNumber)
        let data = await attendance.updateMany(
            { rollNumber: rollNumber },
            { $set: { userName: req.body.userName, rollNumber: rollNumber } }
        )
        if (data) {
            res.send({
                message: 'success',
...
app.post('/paymentToken', async (req, res) => {
    console.log(req.body)
})
app.get('/getAssignments', async (req, res) => {
    let rollNumber = req.query.rollNumber
    let subject = req.query.subject
    let assignmentDetails = await assignment.find({
        $and: [{ rollNumber: rollNumber }, { subject: subject }],
    })
